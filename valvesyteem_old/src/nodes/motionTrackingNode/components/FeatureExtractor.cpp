...
FeatureExtractor::FeatureExtractor(const std::string& enginePath) {
    ...
    // fix: calculate output buffer size and feature dim 
...
bool FeatureExtractor::getApperanceFeatures(const std::vector<cv::Mat>& imagePatches, std::vector<std::vector<float>>& features) {
    ...
    for(int batchIndex = 0; batchIndex < nBatches; ++batchIndex) {
        ...
        if(not preprocessMatBatch(currentBatchPatches, hostInputBuffer)) {
            ROS_ERROR("[FeatureExtractor] Preprocessing failed for batch %d.", batch_idx);
            features.clear();
            return false;   
        }
        ...
        if(cudaMemcpyAsync(gpuBuffers[inputBindingIndex], hostInputBuffer.data(), inputBytes, cudaMemcpyHostToDevice, stream) != cudaSuccess) {
            ROS_ERROR("[FeatureExtractor] CUDA Memcpy H2D failed for batch %d.", batchIndex);
        ...
        if (cudaStatus != cudaSuccess) { ROS_ERROR("[FE] H2D Err: %s", cudaGetErrorString(cuda_status)); cudaStreamSynchronize(stream); features.clear(); return false; }

        bool status = context->enqueueV2(gpuBuffers, stream, nullptr);
        if (!status) { ROS_ERROR("[FE] Inference Err batch %d", batchIndex); cudaStreamSynchronize(stream); features.clear(); return false; }

    
        cudaStatus = cudaMemcpyAsync(hostOutputBuffer.data(), gpuBuffers[outputBindingIndex], outputBytes, cudaMemcpyDeviceToHost, stream);
        if (cudaStatus != cudaSuccess) { ROS_ERROR("[FE] D2H Err: %s", cudaGetErrorString(cuda_status)); cudaStreamSynchronize(stream); features.clear(); return false; }

        cuda_status = cudaStreamSynchronize(stream);
        if (cudaStatus != cudaSuccess) { ROS_ERROR("[FE] Sync Err: %s", cudaGetErrorString(cuda_status)); features.clear(); return false; }
